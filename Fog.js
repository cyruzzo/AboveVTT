...
class WaypointManagerClass {
	...
	drawWaypointSegment(coord, cumulativeDistance, midlineLabels, labelX, labelY) {
		...
		const xOffset = Math.round(Math.abs(snapPointXStart - snapPointXEnd) / gridSize);
		const yOffset = Math.round(Math.abs(snapPointYStart - snapPointYEnd) / gridSize);
		var distance = 0;
		if (window.optional_ruler_rule) {
			const dOffset = Math.min(xOffset, yOffset);
			distance = Math.abs(xOffset - yOffset) + 1.5 * dOffset - 0.5 * (dOffset % 2);
		} else {
			distance = Math.max(xOffset, yOffset);
		}
