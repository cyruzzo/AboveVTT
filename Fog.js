...
function check_single_token_visibility(id){
	...
	var ctx = canvas.getContext("2d");
			...
			if (is_token_under_fog(id) || (playerTokenAuraIsLight && !is_token_under_light_aura(id) && !window.TOKEN_OBJECTS[id].options.revealInFog)) {
				$(selector).hide();
...
function do_check_token_visibility() {
	...
	for (var id in window.TOKEN_OBJECTS) {
		...
		if ((pixeldata[3] == 255 || (playerTokenAuraIsLight && !is_token_under_light_aura(id))) && !window.TOKEN_OBJECTS[id].options.revealInFog) {
			$(selector).hide();
			if(window.TOKEN_OBJECTS[id].options.hideaurafog)
			{
					$(auraSelector).hide();
