...
function inject_monster_image(stat) {
	...
	if (url !== undefined && url != null && url.length > 0 && window.EncounterHandler.combat_body.find(".encounter-details-content-section__content .injected-image").length == 0) {
		...
		let ab = $("<button class='ddbeb-button monster-details-link'>SEND IMAGE TO GAMELOG</button>");				
		let image = `<img style="width:100%" class="injected-image" src="${url}" alt="${stat.data.name}" class="monster-image" onerror="this.src=${stat.data.avatarUrl}"> `;
