...
function reset_zoom () {
	...
	switch (true) {
		case (mapSize > 8000):
			zoom  = zoom + 3 * (stepSize)
			console.log(`map is large applying large ${zoom}`)
			break;
		case (mapSize > 7000):
			zoom  = zoom + 4 * (stepSize)
			console.log(`map is large applying large ${zoom}`)
			break;
        case (mapSize > 6000):
            zoom  = zoom + 5 * (stepSize)
            console.log(`map is large applying large ${zoom}`)
            break;
        case (mapSize > 5000):
			zoom  = zoom + 6 * (stepSize)
			console.log(`map is large applying large ${zoom}`)
			break;
        case (mapSize > 4000):
			zoom  = zoom + 7 * (stepSize)
			console.log(`map is large applying large ${zoom}`)
			break;
        case (mapSize > 3000):
			zoom  = zoom + 8 * (stepSize)
			console.log(`map is large applying large ${zoom}`)
			break;
        case (mapSize > 2000):
            zoom  = zoom + 9 * (stepSize)
            console.log(`map is large applying large ${zoom}`)
            break;
        case (mapSize > 1000):
			zoom  = zoom + 10 * (stepSize)
			console.log(`map is large applying large ${zoom}`)
			break;
		default:
			break;
