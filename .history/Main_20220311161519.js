...
function reset_zoom () {
	...
	heightDiff = 100 * Math.abs( ( vttHeight - sceneMapHeight ) / ( (vttHeight+sceneMapHeight)/2 ) );
	
	vttWidth = $("#VTTWRAPPER")[0].offsetWidth
	sceneMapWidth = $("#scene_map")[0].width
	widthDiff = 100 * Math.abs( ( vttWidth - sceneMapWidth ) / ( (vttWidth+sceneMapWidth)/2 ) );
	zoom = heightDiff > widthDiff ? heightDiff : widthDiff
	console.group("zooming in or out by ", zoom /100)
	change_zoom(zoom/100);
