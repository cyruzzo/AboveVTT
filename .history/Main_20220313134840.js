...
function apply_zoom_from_storage(){
	...
	if (zoomState){
		...
		const zoomIndex = zooms.findIndex(zoom => zoom.title === window.CURRENT_SCENE_DATA.title)
		if(zoomIndex !== -1){
			console.log("skipping application as it doesn't work")
...
function reset_zoom () {
	...
	while($("#scene_map").height() === 0) {
		let observer = new MutationObserver(changes => console.log("I'm going through changes:", changes));
		observer.observe($("#scene_map"), {
