...
function reset_zoom () {
	...
	// loop a few times as each subsequent calculations becomes more accurate
	mapSize = sceneHeight > sceneWidth ? sceneHeight : sceneWidth
	modifier = 1
	switch (true) {
		case (mapSize > 7000):
			modifier = -1.2
			console.log(`map is large applying large ${modifier}`)
			break;
		case (mapSize > 5000):
			modifier = -0.8
			console.log(`map is large applying large ${modifier}`)
		...
		case (mapSize <= 1500):
			modifier = 0.5
			console.log(`map is small applying modifier ${modifier}`)
			break;
	...
	for (let index = 0; index < 5; index++) {
		...
		if(sceneHeight > sceneWidth){
			console.log("using height as primary to calc fit")
			targetSize = sceneHeight + sceneHeight * modifier
			console.log(targetSize)
			zoom = targetSize / vttHeight
		}
		else{
			console.log("using width as primary to calc fit")
			targetSize = sceneWidth + sceneWidth * modifier
			console.log(targetSize)
			zoom = targetSize / vttHeight
		}
		console.log(zoom)
