...
function change_zoom(newZoom, x, y) {
	...
	var centerX = Math.round((($(window).scrollLeft() + zoomCenterX) ) * (1.0 / window.ZOOM));
	var centerY = Math.round((($(window).scrollTop() + zoomCenterY) ) * (1.0 / window.ZOOM));
	window.ZOOM = newZoom;
	var pageX = Math.round(centerX * window.ZOOM - zoomCenterX) ;
	var pageY = Math.round(centerY * window.ZOOM - zoomCenterY) ;
	...
	add_zoom_to_storage(newZoom, pageX, pageY)
	
}

function add_zoom_to_storage(z, x, y){
	const zooms = JSON.parse(localStorage.getItem('zoom')) || [];
	const zoomIndex = zooms.findIndex(zoom => zoom.src === $("#scene_map").attr('src'))
	console.log(zoomIndex)
	if (zoomIndex !== -1){
		console.log("previous zoom data found, updating", zooms[zoomIndex])
...
function get_zoom_from_storage(){
	console.group("get_zoom_from_storage")
	const zoomState = localStorage.getItem("zoom")
	if (zoomState){
		const zooms = JSON.parse(zoomState)
		const zoomIndex = zooms.findIndex(z => z.src === $("#scene_map").attr('src'))
		if(zoomIndex !== -1){
			console.log("does it already know the zoom to apply?", window.ZOOM)
			change_zoom(
...
function reset_zoom () {
	...
	const mH = $("#scene_map")[0].height
	const wW = $(window).width();
	const mW = $("#scene_map")[0].width
