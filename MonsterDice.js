...
function scan_player_creature_pane(target) {
	...
	const clickHandler = function(clickEvent) {
		...
		roll_our_dice(displayName, creatureAvatar, expression, modifier, damageType, false);
	};
	...
	const rightClickHandler = function(contextmenuEvent) {
		...
		let bodyHeight = target.closest("body").height();
		...
		overlay.find("button.MuiButtonBase-root").click(function(clickEvent) {
			...
			console.log(`sendTo: ${sendTo}, rollWith: ${rollWith}`);
			...
			roll_our_dice(displayName, creatureAvatar, expression, modifier, damageType, sendTo == "self");
		});

		target.closest("body").append(overlay);
	}
