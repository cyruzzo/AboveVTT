...
function find_items_in_cache_by_id_and_name(items = []) {
  ...
  for (let item of items) {
    const cachedItem = window.ITEMS_CACHE.find(ci => ci.id.toString() === item.id.toString() && ci.name.toLowerCase().replace(/[^a-z0-9\s]/g, '').includes(item.name.toLowerCase().replace(/[^a-z0-9\s]/g, '')));
    if (cachedItem) {
